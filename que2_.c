/*2. Compute the address of a matrix element for a multidimensional array.
Compute the address of a matrix element for a multidimensional array.
Input – Number of dimensions, Upper bounds of all dimensions, Actual values of the dimensions that the address has to be calculated for Data type of the array

Output – Specific address calculated*/

#include<stdio.h>
#include<stdlib.h>

void getMatrix( int r, int c,int arr[][c]){
	int i,j; 
  for(i = 0; i<r; i++){
    for(j = 0; j<c; j++){
...
void printMatrix_rowMajor(int r, int c,int arr[][c]){
int i,j;
...
void printMatrix_colMajor(int r, int c,int arr[][c]){
int i,j;
...
int main(){
  ...
  scanf("%d%d",&row,&col);
  int matrix[row][col];
  getMatrix(row,col,matrix);
  printMatrix_rowMajor(row,col,matrix);
  printMatrix_colMajor(row,col,matrix);
